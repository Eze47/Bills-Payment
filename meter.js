...
document.getElementById('btn').addEventListener("click", function() {
    console.log(document.getElementById("payElect"))

    document.getElementById("payElect").setAttribute("disabled", "true");
    document.getElementById("payElect").style['backgroundColor'] = 'white';
...
.addEventListener("click", function () {
    ...
    fetch(
      ...
      .catch((error) => {
        ...
        document.getElementById("payElect").disabled = true;
      });
  });

  
  var number='';
document.getElementById('metre').addEventListener('input', function(event){
    number = event.inputType !== "deleteContentBackward" ? number += event.data : number = number.substring(0, number.length - 1)  
    console.log(number)
        
    if (number.length !==11) {
    document.getElementById("payElect").setAttribute("disabled", "true");
    document.getElementById("payElect").style['backgroundColor'] = 'gray';
    document.querySelector(".modal").style.display = "flex";    
  }
    else {
    document.getElementById("payElect").setAttribute("disabled", "false");
    document.getElementById("payElect").style['backgroundColor'] = 'green';
    document.getElementById("payElect").style.cursor = 'pointer';
    }     
